...
cppevent::awaitable_task<void> signal_coroutine(std::unique_ptr<cppevent::event_listener> listener) {
    while(true) {
...
cppevent::task timed_coroutine(cppevent::event_loop& e_loop) {
    ...
    cppevent::awaitable_task<void> t = signal_coroutine(std::move(signal_listener));
